...
| `fm-linear-move.sh`      | Transition a ticket by role (`in-progress`/`in-review`), resolving the role to a workflow-state id by name with a `started`-type fallback for in-progress only |
