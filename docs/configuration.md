...
`LINEAR_INFLIGHT_CAP` (default 1) bounds the ship tasks firstmate runs per repo, and `LINEAR_DRY_RUN` previews a grooming comment to `state/linear-outbox/` instead of posting it.
...
LINEAR_DRY_RUN=         # optional; truthy previews a grooming comment to state/linear-outbox/ instead of posting
