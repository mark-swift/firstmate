...
#                 "LINEARMODE: Linear mode on ..." or "LINEARMODE: Linear mode off ...".
...
#          Linear mode is OPTIONAL and inert unless FM_HOME/.env has a non-empty
#          LINEAR_API_KEY. When opted in, bootstrap requires curl+jq, writes the
#          Linear poll shim and 60s cadence config, and prints a LINEARMODE line.
...
# Linear mode (opt-in): when this home's .env carries a non-empty LINEAR_API_KEY,
# wire the Linear GraphQL poll into the EXISTING watcher check mechanism without
# touching fm-watch.sh or any other watcher-backbone file. Strictly parallel to
# x_mode_setup. Drops two idempotent, gitignored artifacts:
#   state/linear-watch.check.sh - check shim that execs bin/fm-linear-poll.sh
#   config/linear.env           - exports FM_CHECK_INTERVAL=60, sourced by the
#                                 watcher arm so a Linear instance polls at 60s
# On opt-out (no key, or empty) it removes any such artifacts so the instance
# reverts to the default 300s no-poll behavior. Absent a key AND with no leftover
# artifacts it is a complete no-op (nothing written, nothing printed), so a
# non-Linear user sees zero change. It never touches the watcher itself; applying
# a cadence transition to a running watcher is the caller's job via
# 'bin/fm-watch-arm.sh --restart' (see AGENTS.md "Linear mode").
linear_mode_setup() {
  local env_file key shim cadence shim_body cadence_body tool missing
  env_file="$FM_HOME/.env"
  shim="$STATE/linear-watch.check.sh"
  cadence="$CONFIG/linear.env"

  key=
  [ -f "$env_file" ] && key=$(linear_env_get LINEAR_API_KEY "$env_file")

  linear_mode_remove_artifacts() {
    rm -f "$shim" "$cadence" 2>/dev/null || true
    [ ! -e "$shim" ] && [ ! -e "$cadence" ]
  }

  if [ -z "$key" ]; then
    # Opt-out (or never opted in): drop any Linear artifacts; stay silent unless we
    # actually removed something.
    if [ -e "$shim" ] || [ -e "$cadence" ]; then
      if linear_mode_remove_artifacts; then
        echo "LINEARMODE: Linear mode off - removed Linear poll shim and 60s cadence; restart the watcher (bin/fm-watch-arm.sh --restart) to drop back to the default cadence"
      else
        echo "LINEARMODE: Linear mode off - failed to remove Linear poll shim or 60s cadence"
      fi
    fi
    return 0
  fi
  ...
  linear_arm_failed() {
    if linear_mode_remove_artifacts; then
      echo "LINEARMODE: Linear mode off - failed to arm Linear poll shim or 60s cadence"
    else
      echo "LINEARMODE: Linear mode off - failed to arm Linear poll shim or 60s cadence; stale artifacts remain"
    fi
  }

  mkdir -p "$STATE" "$CONFIG" 2>/dev/null || { linear_arm_failed; return 0; }

  shim_body=$(cat <<EOF
#!/usr/bin/env bash
# Auto-generated by fm-bootstrap.sh - Linear mode poll shim.
# The watcher runs this each check cycle; output becomes a check: wake.
export FM_HOME=$(printf '%q' "$FM_HOME")
exec $(printf '%q' "$FM_ROOT/bin/fm-linear-poll.sh")
EOF
)
  write_if_changed "$shim" "$shim_body" || { linear_arm_failed; return 0; }
  chmod +x "$shim" 2>/dev/null || { linear_arm_failed; return 0; }

  cadence_body=$(cat <<'EOF'
# Auto-generated by fm-bootstrap.sh - Linear mode watcher cadence.
# Source this before arming the watcher (see AGENTS.md "Linear mode") so
# fm-watch.sh polls the Linear check every 60s. Non-Linear instances have no such
# file and keep the default 300s cadence.
export FM_CHECK_INTERVAL=60
EOF
)
  write_if_changed "$cadence" "$cadence_body" || { linear_arm_failed; return 0; }

  echo "LINEARMODE: Linear mode on - Linear poll armed via state/linear-watch.check.sh; 60s watcher cadence in config/linear.env"
}
