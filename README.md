...
- **Optional Linear mode** - opt in with one local `.env` key (`LINEAR_API_KEY`) so firstmate uses Linear as a work source: it polls for tickets assigned to its bot user, surfaces them as watcher events (a ready ticket, a canceled ticket, a new grooming comment), and acts on them through the normal lifecycle - risk-gating a ready ticket, dispatching a ship task on the ticket's own branch so the PR auto-links, driving it through In Progress / In Review, handling PR review feedback, and dropping a canceled ticket. The captain gates both ends (what enters the queue and the merge); ticket ideation lands in a later slice. A user who never configures it sees zero behavior change.
...
An opt-in Linear mode likewise uses the watcher check path to poll Linear for tickets assigned to its bot user, surfacing a ready ticket, a newly canceled ticket, or a new grooming comment on a backlog item as a watcher event.
A responder skill then acts on each: it risk-gates a ready ticket and, when clear, dispatches a ship task on the ticket's own Linear branch (so the PR auto-links), moves the ticket through In Progress and In Review, loops it back on PR review feedback, and closes out a canceled ticket; the captain still gates both ends - what enters the queue and the merge - and ticket ideation arrives in a later slice.
